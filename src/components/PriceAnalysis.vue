      ...
      <thead class="thead-dark">
        <tr>
          <th scope="col">前 {{ fetchResultPrice.length }} 笔价格分析
            <br>
            <span v-if="corruptedCount && corruptedCount === fetchResultPrice.length" style="color: lightpink;">{{ corruptedCount }} 笔已污染</span>
            <b-button v-else-if="corruptedCount" @click="$emit('exclude')" size="sm" :variant="corruptedCount > 5 ? 'outline-danger' : 'outline-warning'">排除 {{ corruptedCount }} 笔已污染</b-button>
      ...
      <tbody @mouseleave="hoveredIndex = -1">
        <tr v-for="(item, index) in collectionCurrency" :key="index">
          <td :style="`opacity: ${hoveredIndex == index ? 1 : parseFloat(item.accountName.length / item.count)}; 
            ...
            / <b>{{ item.count }}</b>笔
            <!-- <span v-if="item.count !== item.accountName.length">（<b>{{ item.accountName.length }}</b>人标）</span> -->
            <span v-if="isPriceCollapse === false">（<b>{{ item.accountName.length }}</b>人标）</span>
            <b-icon-x-square v-show="hoveredIndex == index && $store.state.POESESSID" @click="addToBlackList(item.accountName)" v-b-tooltip.hover.right.v-secondary :title="`将这 ${item.accountName.length} 人加入黑名单`" style="cursor: pointer;"></b-icon-x-square>
          </td>
        </tr>
      </tbody>
...
import staticDate from "../assets/poe/static.json";
...
export default {
  ...
  mounted() {
    let vm = this
    // this.axios.get(`https://poe.game.qq.com/api/trade/data/static`, )
    //   .then((response) => { // 通貨 icon
        let response = this.allStatic
        this.Currency = response.result[0].entries
      // })
  ...
  methods: {
    ...
    priceAnalysis(indexLength) {
      ...
      this.axios.all(newFilterResult.map((element, index) => {
          ...
          let url = `${this.baseUrl}/api/trade/fetch/${element}?query=${this.fetchQueryID}`
          return this.axios.post(`http://localhost:3031/api/trade/fetch`,{
            url: url,
        ...
        .catch(function (error) {
          ...
          vm.$message({
            type: 'error',
            message: `被 Server 限制发送请求了，请等待 ${limitState} 秒后在尝试`
          });
        })
    },
      ...
      this.$confirm(`是否将 ${accounts} ${accounts.length > 1 ? `等 ${accounts.length} 人` : ''}加入黑名单?`, '请再次确认', {
        ...
        this.axios.all(accounts.map(element => {
          ...
          .then(vm.axios.spread((...res) => {
            ...
            vm.$message({
              type: 'success',
              message: `已将 ${accounts.length} 人加入黑名单，下次查价时生效！`,
            });
          }))
          .catch(function (error) {
            console.log(error);
      ...
      collectionCurrency.forEach((collection, index) => {
        this.Currency.forEach(element => {
          if (collection.currency === element.id) {
            collectionCurrency[index].image = `https://poe.game.qq.com${element.image}`
